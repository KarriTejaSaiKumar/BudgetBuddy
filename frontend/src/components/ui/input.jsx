...
const Select = React.forwardRef(function Select({ className, children, invalid, ...props }, ref) {
  return (
    <select
      ...
      aria-invalid={invalid || undefined}
      className={cn(
        "flex h-11 w-full appearance-none rounded-xl border border-input bg-surface px-3 py-2 pr-9 text-sm text-foreground shadow-xs transition-[background-color,border-color,box-shadow] duration-200",
